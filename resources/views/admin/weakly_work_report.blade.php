    ...
    <main >
      ...
      <div id="details" class="clearfix">
        <div id="client">
          ...
          <h2 class="name">Database user name:</h2>
    ...
    @endphp
        <thead>
          <tr>
            ...
            <th class="unit">Weight</th>
            <th class="qty">BOX WORK</th>
            <th class="unit">SALARY</th>
          ...
          <tr>
            ...
            <td class="unit">{{$item->workername}} </td>
            <td class="qty">{{$item->typeofproduct}} </td>            
            <td class="unit">{{$item->weight}} </td>
            <td class="qty">{{$item->workbybox}} </td>
            <td class="unit">{{$item->totalprice}} </td>
            <td class="qty">{{$item->pymentbyfactory}} </td>
            <td class="unit">{{$item->totalprice-$item->pymentbyfactory}} </td>
            <td class="qty">{{$item->date}} </td>
            
         
            @php
            $totalcost +=$item->totalprice; // Add the cost of each item to the total cost
            $totalpyment += $item->pymentbyfactory;
            $totaldebate += $item->totalprice-$item->pymentbyfactory;
           @endphp
          </tr>
          @endforeach
          </tbody>
          <tfoot>
            <tr>
                <tr>
                    <td colspan="6"></td>
                    <td colspan="1">Total weakly 
                    </td>
                    <td>{{$totalpyment}}</td>
                  </tr>

              <td colspan="6"></td>
              <td colspan="1">Total Weakly cost</td>
              <td>AFN {{$totalcost}}</td>
            </tr>
            <tr>
              <td colspan="6"></td>
